...
class ECKeyp:
    def __init__(self, pkey=None, pubkey=None, curve="secp256r1"):
        self.pkey = pkey
        self.pubkey = pubkey
        if not pkey and not pubkey:
            if curve not in _mbed.ec_curves():
                ...
                self.pkey, self.pubkey = _mbed.ec_gen_key(curve)
        else:
    ...
    def derive_pubkey(self):
        self.pubkey = _mbed.ec_get_pubkey(self.pkey)

    def sign(self, data):
        if self.pkey:
            return _mbed.ec_key_sign(self.pkey, data)
        else:
    ...
    def verify(self, data, sig):
        try:
            if self.pubkey:
                return _mbed.ec_key_verify(self.pubkey, data, sig)
            else:
